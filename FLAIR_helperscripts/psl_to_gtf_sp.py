...
def split_iso_gene(iso_gene):
	...
	#elif '&' in iso_gene:
	#	iso_gene = iso_gene[:iso_gene.find('&')]
	#	iso = iso_gene[:iso_gene.find('_')]
	#	gene = iso_gene[iso_gene.find('_')+1:]
	else:
...
for line in open(args.psl):
	...
	print('\t'.join([chrom, 'sp_collapse', 'transcript', str(start+1), str(tstarts[-1]+bsizes[-1]), '.', strand, '.',
		attributes]))
	if isbed and thick_start != thick_end and (thick_start != start or thick_end != end):
		print('\t'.join([chrom, 'sp_collapse', 'CDS', str(thick_start+1), str(thick_end), '.', strand, '.',
		attributes]))
		if strand == '+':
			print('\t'.join([chrom, 'sp_collapse', 'start_codon', str(thick_start+1), str(thick_start+3), '.', strand, '.',
			attributes]))
			print('\t'.join([chrom, 'sp_collapse', '5UTR', str(start+1), str(thick_start+1), '.', strand, '.',
			attributes]))
			print('\t'.join([chrom, 'sp_collapse', '3UTR', str(thick_end), str(tstarts[-1]+bsizes[-1]), '.', strand, '.',
			attributes]))
		elif strand == '-':
			print('\t'.join([chrom, 'sp_collapse', 'start_codon', str(thick_end-2), str(thick_end), '.', strand, '.',
			attributes]))
			print('\t'.join([chrom, 'sp_collapse', '3UTR', str(start+1), str(thick_start+1), '.', strand, '.',
			attributes]))
			print('\t'.join([chrom, 'sp_collapse', '5UTR', str(thick_end), str(tstarts[-1]+bsizes[-1]), '.', strand, '.',
			attributes]))
	if strand == '-':  # to list exons in 5'->3'
		for b in range(len(tstarts)):  # exon number
			...
			attributes = 'gene_id \"{}\"; transcript_id \"{}\"; exon_number \"{}\";'\
							.format(gene_id, transcript_id, b+1)
			print('\t'.join([chrom, 'sp_collapse', 'exon', str(tstarts[bi]+1), \
		...
		for b in range(len(tstarts)):
			...
			if args.reference_transcript_id and '-referencetranscript' in transcript_id:		
				...
				.format(gene_id, trimmed_transcript_id, b+1, trimmed_transcript_id)
			print('\t'.join([chrom, 'sp_collapse', 'exon', str(tstarts[b]+1), \
